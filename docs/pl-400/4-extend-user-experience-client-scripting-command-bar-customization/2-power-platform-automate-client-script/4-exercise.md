...
[![Screenshot of the select components button.](media/edit-objects.png)](media/edit-objects.png#lightbox)
...
[![Screenshot of the select components link.](media/edit-objects-button.png)](media/edit-objects-button.png#lightbox)
...
17. Enter **Investment Required** for **Display name**, select **Choice | Yes/No** for **Data type**, expand the **Advanced options**. Make a note of the **prefix** of the **Schema name**; your prefix is different. You need this when you're creating the script and what to refer to the fields you're adding.
...
26. The form should now look like this image. Select **Save and publish**. Wait for the publishing to be completed.
...
10. Add this function to the **IdeaForm** file. This function is called from both OnLoad and OnChange functions and the formContext is passed as an argument. This is where you implement the business logic to set the requirement levels.
    ...
    ```
11. You first get the challenge lookup values. Add this script to the **setRequiredFields** function. We got `sample_originatingchallengeid` from the properties of the column in solution explorer.
...
13. You make the columns not required if the challenge is null. Add this script inside the **else**. This ensures if a challenge isn't selected the columns aren't required.
    ...
    ```
14. Your **IdeaForm** should now look like this image.

[![Screenshot of the ideaform with added script.](media/idea-form.png)](media/idea-form.png#lightbox)
15. You get the ID of the related **challenge** if the challenge isn't null. Add this script inside the **if** statement.

    ```javascript
    var challengeId = challenge[0].id;
    ```
16. Now you retrieve the challenge row using **WebAPI** and select only the two columns you're interested in **InvestmentRequired** and **ROIRequired**. Add the script inside if statement.
    ...
    ```
17. If the **retrieveRecord** succeeds, you first create two variables that hold the requirement values for both columns and set their initial value to not required. After that you check whether the columns are required and set the requirement variable accordingly. Finally you set the column requirement to the value of the variable you created. Add this script inside the success function. Replace the prefix **cr07c\_** for cr07c\_investmentrequired and cr07c\_roirequired with the prefix of the columns you created.
    ...
    ```
18. You log the error message if **retrieveRecord** causes an error. Add this script inside the error function. You could also use methods from Xrm.Navigation to show a dialog to the user with appropriate options if necessary.

    ```javascript
    console.log(error.message);
    ```
19. The **LearnLab**\_**setRequiredFileds** function should now look like this image.

[![Screenshot of the completed function.](media/completed-function.png)](media/completed-function.png#lightbox)
20. You call the **LearnLab**\_**setRequiredFileds** from both **OnLoad** and **OnChange** functions. Add this script to both **OnLoad** and **OnChange** functions.

    ```javascript
    LearnLab_setRequiredFields(formContext)
    ```
21. Your **IdeaForm** should now look like this image.

[![Screenshot of the completed idea form script.](media/completed-idea.png)](media/completed-idea.png#lightbox)
22. Select **File** and **Save all**.
...
In this task, you add the JavaScript library to the Idea main form and add an event handler for On Load event.
...
9. Enter **LearnLab\_handleIdeaOnLoad** for **Function**, check the **Pass execution context as first parameter** checkbox, and select **Done**.
