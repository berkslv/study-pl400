...
It supports various command-line arguments that you can use to perform specific actions as part of your solution extract/pack operation. For in-depth details on how to use the SolutionPackager tool, see the [SolutionPackager Tool](/en-us/power-apps/developer/common-data-service/compress-extract-solution-file-solutionpackager/?azure-portal=true) article.

Sometimes it's necessary to manage items, such as web resources or plug-in assemblies that are created and managed in separate projects. This requires mapping solution files from their default location in an extract folder to their corresponding project directory. For example, a plug-in assembly will typically be located in the bin output folder of the plug-in assembly project. To accomplish this, you can use the /map argument to manage the mapping of these locations. For a detailed explanation on how to build an XML mapping document, see [Use the /map Command Argument](/en-us/power-apps/developer/common-data-service/compress-extract-solution-file-solutionpackager?azure-portal=true#use-the-map-command-argument).
